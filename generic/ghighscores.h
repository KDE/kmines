 ...
 *
 * To submit a new score at game end, just call @ref newScore then set the
 * score data with @ref setScoreData and finally call @ref submitScore.
 * <pre>
 *     KExtHighscores::Score *score;
 *     score = kHighscores->newScore(KExtHighscores::Won);
 *     score.setData("score", myScore);
 *     kHighscores->submitScore(widget); // delete score
 * </pre>
...
class Highscores
{
 public:
     ...
     *
     * You need to create a score by player with @ref newScore
     * and then feed them to this method.
     * Note that you are responsible for deleting the @ref Score objects.
     *
     * <pre>
     * QPtrVector<Score> scores(2);
     * scores[0] = kHighscores->newScore(KExtHighscores::Won);
     * scores[0]->setData("score", score1);
     * scores[0]->setData("name", player1);
     * scores[1] = new Score(KExtHighscores::Won);
     ...
     * </pre>
     */
    static void showMultipleScores(const QPtrVector<Score> &scores,
                                   QWidget *parent);

    /**
     * You need to call this method before submitting a score
     * (with @ref submitScore) to get a @ref Score instance.
     *
     * @param type @ref Lost and @ref BlackMark are only allowed if you
     * have enabled the lost/black mark game tracking at construction time.
     */
    Score *newScore(ScoreType type);
     ...
     */
    void submitScore(QWidget *widget);
 ...
 protected:
    ...
    /**
     * @return true is the first score is better than the second one.
     ...
     */
    virtual bool isStrictlyBetter(const Score &score1,
    ...
    /**
     * Replace the default @ref Item named "score" (@ref ScoreItem) by the
     * given one.
     */
    void setScoreItem(Item *item);

    /**
     * Replace the default @ref Item named "best score" (@ref BestScoreItem)
     * by the given one.
     */
    void setBestScoreItem(Item *item);

    /**
     * Replace the default @ref Item named "mean score" (@ref MeanScoreItem)
     * by the given one.
     */
    void setMeanScoreItem(Item *item);
     ...
     * if they are known.
     */
    void submitLegacyScore() const { submitLocal(); }
 ...
 private:
    ...
    int rank() const; // return -1 if not a local best score

    int submitLocal() const;
    void submitWorldWide(QWidget *parent);
    ...
    friend class HighscoresSettingsWidget;
